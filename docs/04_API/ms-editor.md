...
Назначение: конфигурация объектов, мнемосхем, фигур и публикация изменений для `editor-ui`.

## Текущий статус skeleton

Фактически в коде `ms-editor` на текущий момент заведены только:

- объекты мониторинга;
- мнемосхемы;
- фигуры.

Устройства, теги и справочники остаются частью целевой архитектуры, но ещё не реализованы в HTTP-слое сервиса. Ниже они сохранены как целевой scope, чтобы не потерять контракт Phase 2.
...
Фигуры
- `POST /diagrams/{id}/figures` - добавить фигуры (append)
- `PUT /diagrams/{id}/figures` - bulk upsert/replace фигур схемы (одной транзакцией)
- `GET /diagrams/{id}/figures` - список фигур схемы
...
- Ответ: `id`, `diagram_id`, `type`, `tag_id`, `params`, `created_at`, `updated_at`.

Bulk upsert фигур (`PUT /diagrams/{id}/figures`)
- Запрос: `{ "figures": [ { "id"?, "type", "tag_id"?, "params" }, ... ] }`.
  - Элемент с `id` — обновить существующую фигуру в этой мнемосхеме.
  - Элемент без `id` — создать новую фигуру с привязкой к диаграмме из пути.
  - Фигуры мнемосхемы, отсутствующие в теле, — soft-delete (проставить `deleted_at`).
- Семантика: идемпотентная операция «привести состав фигур мнемосхемы к переданному списку» в одной транзакции.
- Ответ: `{ "figures": [...], "created": N, "updated": N, "deleted": N }` — отражает итог операции.
- События: по одному `config.changed` на каждую затронутую фигуру с `operation ∈ {created, updated, deleted}`. Публикация — после успешного commit, сбой публикации не откатывает транзакцию (логируется).
- Ошибки: 400 — невалидный JSON; 404 — мнемосхема не найдена или `id` фигуры не принадлежит мнемосхеме; 422 — неподдерживаемый `type` / невалидный `params` / дубликаты `id` в теле.
