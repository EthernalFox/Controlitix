...
| `docs/` | После итерации 1 есть: НФТ, ADR, архитектура с mermaid, модель данных (базовая), глоссарий, сценарии (черновые). **Нет дизайна и UX**. |
...
**Главная проблема:** дизайн отсутствует полностью. `docs/06_Дизайн/README.md` — 4 строки placeholder-а. В команде нет ни wireframes, ни визуального языка HMI, ни компонентной библиотеки поверх Mantine. Разработка `editor-ui` уже идёт на базе догадок. Если не закрыть дизайн-фазу до старта `viewer-ui` и до активной работы над canvas-редактором, будут массовые переделки.
